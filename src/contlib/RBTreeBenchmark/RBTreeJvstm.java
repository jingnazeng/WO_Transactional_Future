...
public class RBTreeJvstm {
	...
	public RBTreeJvstm() {
		...
		aux.setLeft(sentinelNode);

		aux.setValue(Integer.MIN_VALUE);

		aux.setColor(Color.BLACK);

		root = new VBox<RBNode>(aux);
...
//meter aki um try acatch para ver info sobre o node (k tentative versions existem)
		if(node.getParent() != null) {//its null
	...
	public static class RBNode {
		...
		private VBoxInt value = new VBoxInt();

		private VBoxBoolean isMarked = new VBoxBoolean();

		private VBox<Color> color = new VBox<Color>();

		private VBox<RBNode> parent = new VBox<RBNode>();

		private VBox<RBNode> left = new VBox<RBNode>();

		private VBox<RBNode> right = new VBox<RBNode>();
