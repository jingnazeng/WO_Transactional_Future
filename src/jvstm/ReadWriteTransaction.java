...
public abstract class ReadWriteTransaction extends Transaction {
    ...
    protected Transaction treeRoot;
    ...
    public ReadWriteTransaction(int number) {
        ...
        this.boxesWritten = new ConcurrentHashMap<VBox, Object>();
    }
    ...
    //added
    public boolean safeToCreateReadOnly(int seqNumber) {
    	int RWFuture = -1;
    	
    	for(Integer RW : getTopLevelParent().sequentialRWVersion) {
    		if( (RW%2 != 0)  && (RW < seqNumber - 1) ) {
    			RWFuture = RW;
    			break;
    		}
    	}
    	if(RWFuture == -1) return true;
    	
//    	if(getTopLevelParent().seqRWClock >= RWFuture)
//    		return true;
//    	else
//    		return false;

    	Transaction ancestor = getParent();
    	
    	while( ancestor != null && ancestor.sequentialVersion != RWFuture - 1) {
    ...
    public Transaction makeWeakOrderingTransactionalFuture(boolean readOnly){
    	Transaction newTx = null;
        if (readOnly) {
        	newTx = new ReadOnlySubTxn(this);
        } else {
        	try{
        	newTx = new WeakOrderingFutureSubTransaction(this);
        	}catch(ReleaseWriteLockErrorException e){
        		e.printStackTrace();
        	}
        }
        return newTx;
    ...
    public Transaction makeParallelNestedTransaction(boolean readOnly,int seqNumber) {
        if (readOnly && safeToCreateReadOnly(seqNumber)) {//added makes a future
            return new ParallelNestedReadOnlyTransaction(this,seqNumber,null);
        }
        getTopLevelParent().sequentialRWVersion.add(seqNumber);
        return new ParallelNestedTransaction(this, seqNumber,null);
    }

    @Override
    public Transaction makeParallelNestedTransaction(boolean readOnly) {
    	Transaction newTx;
        if (readOnly) {//added makes a continuation
        	newTx = new ParallelNestedReadOnlyTransaction(this,this.sequentialVersion+2, nextSpeculationCheckpoint);
        } else {
        	newTx = new ParallelNestedTransaction(this, this.sequentialVersion+2,nextSpeculationCheckpoint);
        }
    	nextSpeculationCheckpoint = null;
        return newTx;
    }
    ...
    // added
    protected ReadWriteTransaction getTopLevelParent() {
        Transaction nextParent = getRWParent();
		if(nextParent == null) return this;
        while (nextParent.parent != null) {
            nextParent = nextParent.parent;
        }
        return (ReadWriteTransaction) nextParent;
    }
    
	
    @Override
    public <V> V evalWeakOrderingFuture(FutureRepresentation<V> tf) throws InterruptedException, ExecutionException {
    	
    	DAG dagFortxn = this.getTopLevelTrasanction().getDAG();
    	
    	Node contiNode = this.getAssociatedNode();
    	Node evalNode =  dagFortxn.futureEvaluationCreateEvalNode(tf.getFuture_node(),contiNode);
    	
    	evalNode.setResidingDAG(this.getAssociatedNode().getResidingDAG());
    	//associate the evaluation sub-transactions with corresponding nodes in DAG
    	Transaction evalTxn = Transaction.begin();
    	evalNode.setAsscociatedTxn(evalTxn);
    	evalTxn.setAssociatedNode(evalNode);
//    	System.out.println("milestone TWO for txn:  "+ this.getTopLevelTrasanction());
    	dagFortxn.futureEvaluationSetEvalNode(tf.getFuture_node(), contiNode, evalNode);
//    	System.out.println("milestone THREE for txn:  "+ this.getTopLevelTrasanction());
//    	dagFortxn.getGraphLock().readLock();
//    	System.out.println("evaluation of future and the dag is now: \n" + dagFortxn.getGraph() );
//    	dagFortxn.getGraphLock().readUnlock();
    	
//    	V result = tf.getF_ref().get();
//   	return result;
    	return null;
    ...
    protected void finish() {
        ...
        for(VBox boxWritten: boxesWritten.keySet()){
        	boxWritten.releaseExclusiveLock();
        }
...
//        	System.out.println("released lock by transaction: " + this);
        //	System.out.println("released lock on Vbox: " + boxWritten);
//        	System.out.println("----------------------------- " );
        }
        ...
        //boxesWrittenInPlace = new HashSet<VBox>();
        perTxValues = EMPTY_MAP;
        arrayWrites = EMPTY_MAP;
        arrayWritesCount = EMPTY_MAP;
    }

    private void iCommitEvalNode() {
    	if(Transaction.current() instanceof WeakOrderingFutureSubTransaction){
    		DAG dagFortxn = Transaction.current().getTopLevelTrasanction().getDAG();
    		Node evalNode = Transaction.current().getAssociatedNode();
    		if(evalNode.getType() != Node.Type.evaluation){
    			return;
    		}else{
    			dagFortxn.concludeLastEval(evalNode);
    		}
    	}else{
    		return;
    	}
...
//         * is not greater than my version we know that this transaction (as well
...
//    }
    ...
    public <T> T getBoxValue(VBox<T> vbox) {
         ...
         ReadWriteTransaction currentOwner = vbox.getWriteLock().getOwner_top_level_transaction_id();
        if (currentOwner!= this.getTopLevelParent()) {
            return readFromBody(vbox);
        } else {
            T value = getLocalValue(vbox);
            if (value == null) { // no local value exists
