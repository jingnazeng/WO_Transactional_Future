...
/**
 * RW conflict between future and the continuation
 * in this version, read write conflict is created between the future and its continuation,
 * therefore, future has to be serilzied at evaluation point 
 * **/

public class BaseTransactionV2 implements ArrayAccessTransaction{
	...
	public BaseTransactionV2(int read, int write,String high_contention, int amount, boolean seqExe){
	...
	public int executeTransaction(Transaction tx, int sibling, int streaming) throws Throwable {
		...
		if(seqExe){
			...
			writeToPariticularObject();
			readPariticularObject();
			int value_read = read();
			write(value_read);
	...
	private void writeToPariticularObject() {
		int index = 0;
		array[index].put((int)(Math.random()*1000)+value);
	}
	
	private void readPariticularObject() {
		int index = 0;
